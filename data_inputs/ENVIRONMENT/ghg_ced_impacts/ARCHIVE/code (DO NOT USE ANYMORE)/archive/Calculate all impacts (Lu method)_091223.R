...
# any missing ratios?
both_days15 %>% filter(is.na(ratio_FAFH_FAH)) #none-good
...
# dat_wide2 <- dat_wide1 %>% select(seqn, fcidcode, avg_intake)
...
ids <- both_days15 %>% select(seqn) %>% distinct() %>% mutate(foo = 1)

fcids <- both_days15 %>% select(fcidcode, fcid_desc) %>% distinct() %>% mutate(foo = 1)
...
fcid_long4 <- fcid_long3 %>% mutate(CED = (CED_mn / 1000) * intake_mean,
                                    GHG = (GHG_mn / 1000) * intake_mean) %>% 
...
fcid_table <- fcid_long5 %>% 
  ...
  summarise(groupintake = sum(intake_mean),
            groupGHG = sum(GHG),
            groupCED = sum(CED))
...
fcid_table1 <- fcid_table %>% 
  mutate(CEDper100gram = (groupCED / groupintake) * 100,
         GHGper100gram = (groupGHG / groupintake) * 100)

# Export
write_csv(fcid_table1, "in/Environmental impact (Brooke)/clean data/Impacts_total_enviro_LU_091323.csv")
...
# IMPORT FPED
fped1516 <- read_sas("in/Dietary intake (Brooke)/raw data/fped_1516.sas7bdat")
...
cost1516 <- read_xlsx("in/Food prices (Brooke)/raw data/NHANES Foods National Average Prices_InfoScan only (old).xlsx",
                      sheet = "NHANES1516_IRI2015_2016_retail") %>% 
  select(FoodCode, price_100gm) %>% 
...
# IMPORT DR1IFF
dr1iff_i <- read_sas("in/Dietary intake (Brooke)/raw data/dr1iff_i (Lu).sas7bdat") %>% 
  select(DR1IFDCD, DR1IGRMS) %>% 
  rename(dr12igrms = DR1IGRMS,
         FoodCode = DR1IFDCD)

dr2iff_i <- read_sas("in/Dietary intake (Brooke)/raw data/dr2iff_i (Lu).sas7bdat") %>% 
  select(DR2IFDCD, DR2IGRMS) %>% 
  rename(dr12igrms = DR2IGRMS,
         FoodCode = DR2IFDCD)

# combine
dr12iff <- rbind(dr1iff_i, dr2iff_i)
...
my_table <- left_join(fped1516_2, FCintake, by = "FoodCode")
...
mod_results1 <- mod_results %>% rename(unitprice = estimate,
                       unitprice_se = std.error,
                       food_group = term) %>% 
...
write_csv(mod_results1, "in/Environmental impact (Brooke)/clean data/Impacts_total_price_Lu_091323.csv")
