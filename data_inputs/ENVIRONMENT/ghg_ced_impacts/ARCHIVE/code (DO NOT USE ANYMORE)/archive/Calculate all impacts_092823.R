...
# ~ THIS IS WHERE I LEFT OFF FOR THE NIGHT 9/28/23 ~
...
price_impact_split <- my_price_table1 %>% 
  ...
  summarise(price_per_day = sum(price_impact_per_foodcode),
            purchase_per_day = sum(purchased_amt_FNDDS)) %>% 
  filter(!(is.na(foodcode))) %>% 
  replace(is.na(.), 0)
...
price_wide_split <- pivot_wider(price_impact_split, 
                          ...
                          values_from = c(price_per_day, purchase_per_day)) %>% 
  replace(is.na(.), 0) 
...
price_wide_split1 <- price_wide_split %>% 
  ...
  rowwise() %>% 
  mutate(#FAH (Grocery)
         price_impact_fah_avg = mean(c(price_per_day_Grocery_1, price_per_day_Grocery_2)),
         purchase_fah_avg = mean(c(purchase_per_day_Grocery_1, purchase_per_day_Grocery_2)),
         
         #FAFH (Other)
         price_impact_fafh_avg = mean(c(price_per_day_Other_1, price_per_day_Other_2)),
         purchase_fafh_avg = mean(c(purchase_per_day_Other_1, purchase_per_day_Other_2))) %>% 
  select(seqn, foodcode, price_impact_fah_avg, purchase_fah_avg, price_impact_fafh_avg, purchase_fafh_avg)
...
map <- both_days24 %>% select(foodcode, Foodgroup_FNDDS) %>% distinct()
...
price_wide_comb2 <- price_wide_comb1 %>% 
  ...
  summarise(price_total_sum = sum(price_impact_total_avg),
            purchase_total_sum = sum(purchase_total_avg),
            
            price_fah_sum = sum(price_impact_fah_avg),
            purchase_fah_sum = sum(purchase_fah_avg),
            
            price_fafh_sum = sum(price_impact_fafh_avg),
            purchase_fafh_sum = sum(purchase_fafh_avg))

# transform to wide
price_wide_comb3 <- price_wide_comb2 %>% 
  pivot_wider(id_cols = seqn,
              names_from = Foodgroup_FNDDS,
              values_from = c(price_total_sum, purchase_total_sum, 
                              price_fah_sum, purchase_fah_sum,
                              price_fafh_sum, purchase_fafh_sum)) %>% 
  replace(is.na(.), 0)
...
my_enviro_table <- 
  both_days24 %>% 
  mutate(
    GHG = (GHG_mn / 1000) * purchased_amt_FCID,
    CED = (CED_mn / 1000) * purchased_amt_FCID)
...
fcid_intake_dat1 <- fcid_intake_dat %>% 
  ...
  rename(fcid_purchased_mean = intake_impact_mean,
         fcid_purchased_se = intake_impact_se)
...
enviro_final_dat2 <- enviro_final_dat1 %>% 
  rowwise() %>% 
  mutate(GHGper100gram = ifelse(fcid_purchased_mean == 0, 0, (ghg_impact_mean / fcid_purchased_mean) * 100),
         CEDper100gram = ifelse(fcid_purchased_mean == 0, 0, (ced_impact_mean / fcid_purchased_mean) * 100),
...
cost_final_dat2 <- cost_final_dat1 %>% 
  ...
  mutate(costper100gram = ifelse(fndds_intake_mean == 0, 0, (price_mean / fndds_intake_mean) * 100),
         costperDGA = costper100gram * (Conversion_to_grams / 100)) %>% 
  select(-Conversion_to_grams)

# MERGE COST AND ENVIRO DATASETS

all_impacts <- left_join(enviro_final_dat2, cost_final_dat2, by = c("subgroup", "food"))

# EXPORT
# all impacts
# write_csv(all_impacts, "in/Environmental impact (Brooke)/clean data/Impacts_total_BROOKE_091423.csv")

# Just enviro impacts
write_csv(enviro_final_dat2, "in/Environmental impact (Brooke)/clean data/Impacts_enviro_BROOKE_091423.csv")

# Just price
write_csv(cost_final_dat2, "in/Environmental impact (Brooke)/clean data/Impacts_price_BROOKE_091423.csv")
