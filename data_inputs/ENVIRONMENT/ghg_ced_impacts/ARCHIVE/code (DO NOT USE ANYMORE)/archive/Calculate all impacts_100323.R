...
cost_final_dat2 <- cost_final_dat1 %>% 
  ...
  select(subgroup, food, food_type, starts_with(c("costper100gram", "costperDGA")))
...
enviro_final_dat2 <- enviro_final_dat1 %>% 
  ...
  select(subgroup, food, starts_with(c("GHGper", "CEDper")))
...
# EXPORT
# all impacts
write_csv(all_impacts, "in/Environmental impact (Brooke)/clean data/Impacts_total_BROOKE_100423.csv")
