...
# look at rows where there is any processed meat
...
food_cats <- read_csv("in/FCID to diet/data/FCID_food_category.csv")
...
both_days2 %>% filter(fcidcode %in% meat_ids) %>% View()

# create fcid_meat_yes var
both_days3 <- both_days2 %>% mutate(fcid_meat_yes = ifelse((fcidcode %in% meat_ids) & pf_meat > 0, 1, 0)) 
...
both_days4 %>% filter(pm_pct > 0 & pm_pct < 1) %>% View()

both_days5 <- both_days4 %>% mutate(commodity_wt_redm = wt * redm_pct,
                          commodity_wt_pm = wt * pm_pct,
                          commodity_wt_poultry = wt * poultry_pct)


# calculate intake for meat FCIDs

both_days6 <- 
  both_days5 %>% 
  mutate(pf_redm_fcid_intake_grams = ifelse(fcid_meat_yes == 1, grams * commodity_wt_redm / 100, 0),
         pf_pm_fcid_intake_grams = ifelse(fcid_meat_yes == 1, grams * commodity_wt_pm / 100, 0),
         pf_poultry_fcid_intake_grams = ifelse(fcid_meat_yes == 1, grams * commodity_wt_poultry / 100, 0))

# create dummy vars
both_days7 <- both_days6 %>% 
  mutate(pf_redm_fcid_yes = ifelse(pf_redm_fcid_intake_grams > 0, 1, 0),
         pf_pm_fcid_yes = ifelse(pf_pm_fcid_intake_grams > 0, 1, 0),
         pf_poultry_fcid_yes = ifelse(pf_poultry_fcid_intake_grams > 0, 1, 0),
         
         # calculate intake amount for each FCID
         consumed_amt_g = grams * (wt / 100))

# identify rows where FCID is both processed meat AND other kind of meat
both_days7 %>% filter(pf_pm_fcid_yes == 1 & (pf_redm_fcid_yes == 1 | pf_poultry_fcid_yes ==1)) %>% View()
...
# merge with bothdays
both_days8 <- left_join(both_days7, losswaste, by = "fcidcode")
...
# merge
both_days9 <- left_join(both_days8, new_map, by = "fcidcode")
...
both_days10 <- both_days9 %>% filter(Foodgroup %in% diet_factors | is.na(Foodgroup))

# which ones have missing fcid?
both_days10 %>% filter(is.na(fcidcode)) %>% View()
both_days10 %>% filter(is.na(fcidcode)) %>% select(foodcode, description) %>% distinct() %>% View()

# which ones have missing food group?
both_days10 %>% filter(is.na(Foodgroup)) %>% select(foodcode, description) %>% distinct() %>% View()

# export
no_fcids <- both_days10 %>% filter(is.na(Foodgroup)) %>% select(foodcode, description) %>% distinct()
# come back and manually assign these to FCIDs/foodgroups

# for now, if missing FCID, remove from dataset
both_days11 <- both_days10 %>% filter(Foodgroup %in% diet_factors)
...
both_days13 <- left_join(both_days12, fafh, by = c("Foodgroup" = "Diet_var"))
...
# now merge
both_days14 <- left_join(both_days13, datafield_sub, by = "fcidcode")
...
# combine with food data
both_days16 <- left_join(both_days15, price_comb, by = c("foodcode" = "food_code", "nhanes1516")) %>% 
  mutate(foodcode = as.character(foodcode))

write_rds(both_days16, "in/Environmental impact (Brooke)/clean data/All_impacts_by_fcidcode_08042023.rds")
...
# THIS CODE IS SO INEFFICIENT - NEED TO REWRITE

# STOPPED HERE 8-4-23


# how many fndds codes don't have food group?
both_days17 %>% filter(is.na(fndds_food_group)) %>% select(foodcode, description) %>% distinct() %>% View()
