...
datafield_water <- read_xlsx("in/Environmental impact (Brooke)/raw data/dataFIELD_water public v1.xlsx",
                             sheet = "FCID Codes",
                             skip = 2)
...
mixed <- read_csv("in/Environmental impact (Brooke)/raw data/mixed_dishes_100923.csv") %>% 
...
price_wide_comb <- full_join(price_wide_total1, price_wide_split1, by = c("seqn", "foodcode", "mixed_dish"))
...
enviro_impact <- my_enviro_table %>% 
  ...
  summarise(GHG_per_day_Consumed = sum(GHG_impact_per_FCID_Consumed),
            ...
            WATER_per_day_Consumed = sum(WATER_impact_per_FCID_Consumed),
            
            BLUEWATER_per_day_Consumed = sum(BLUEWATAER_impact_per_FCID_Consumed),
...
enviro_wide1 <- enviro_wide %>% 
  ...
  mutate(ghg_impact_avg_Consumed = mean(c(GHG_per_day_Consumed_1, GHG_per_day_Consumed_2)),
         ...
         water_impact_avg_Consumed = mean(c(WATER_per_day_Consumed_1, WATER_per_day_Consumed_2)),
         
         bluewater_impact_avg_Consumed = mean(c(BLUEWATER_per_day_Consumed_1, BLUEWATER_per_day_Consumed_2)),
...
# join
enviro_wide5 <- full_join(enviro_wide4, subgroup_dat, by = c("seqn" = "SEQN"))
...
# Define survey design for ghg dataset 
my_enviro_svy <- svydesign(data=enviro_wide5, 
                           id=~SDMVPSU, # Masked Variance Unit Pseudo-PSU 
                           strata=~SDMVSTRA, # Masked Variance Unit Pseudo-Stratum 
                           weights=~wtnew, # New sample weight
                           nest=TRUE)
...
allfoods_ghg_bysub <- svyby(reformulate(names(enviro_wide5) %>% str_subset("ghg")),
                            ~subgroup,
                            my_enviro_svy_sub,
                            svymean)

# CALCULATE CED IMPACT
allfoods_ced_bysub <- svyby(reformulate(names(enviro_wide5) %>% str_subset("ced")),
                            ~subgroup,
                            my_enviro_svy_sub,
                            svymean)

# CALCULATE WATER IMPACT
allfoods_water_bysub <- svyby(reformulate(names(enviro_wide5) %>% str_subset("^water")),
                            ~subgroup,
                            my_enviro_svy_sub,
                            svymean)

# CALCULATE BLUEWATER IMPACT
allfoods_bluewater_bysub <- svyby(reformulate(names(enviro_wide5) %>% str_subset("bluewater")),
                              ~subgroup,
                              my_enviro_svy_sub,
                              svymean)

# CALCULATE CONSUMED, INEDIBLE, WASTED AMOUNTS
allfoods_enviro_consumed_bysub <- svyby(reformulate(names(enviro_wide5) %>% str_subset("consumed")),
                                    ~subgroup,
                                    my_enviro_svy_sub,
                                    svymean)

allfoods_enviro_wasted_bysub <- svyby(reformulate(names(enviro_wide5) %>% str_subset("wasted")),
                                        ~subgroup,
                                        my_enviro_svy_sub,
                                        svymean)

allfoods_enviro_inedible_bysub <- svyby(reformulate(names(enviro_wide5) %>% str_subset("inedible")),
                                      ~subgroup,
                                      my_enviro_svy_sub,
                                      svymean)
...
# EXPORT
# all impacts
write_csv(all_impacts, "in/Environmental impact (Brooke)/clean data/Impacts_total_BROOKE_100923.csv")

# just cost
write_csv(cost_final_dat2, "in/Environmental impact (Brooke)/clean data/Impacts_cost_BROOKE_100923.csv")

# just enviro
write_csv(enviro_final_dat2, "in/Environmental impact (Brooke)/clean data/Impacts_enviro_BROOKE_100923.csv")
