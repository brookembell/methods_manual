...
my_health_dat1 <- my_health_dat %>% 
  ...
  mutate(impact_median_day = NA,
         LB_day = NA,
         UB_day = NA,
         combined_consumed_impact_median_day = NA,
         LB_consumed_day = NA,
         UB_consumed_day = NA,
         combined_unconsumed_impact_median_day = NA,
         LB_unconsumed_day = NA,
         UB_unconsumed_day = NA,
         combined_consumed_impact_median_year = NA,
         LB_consumed_year = NA,
         UB_consumed_year = NA,
         combined_unconsumed_impact_median_year = NA,
         LB_unconsumed_year = NA,
         UB_unconsumed_year = NA) %>% 
...
for (i in costenv_both_folder_name) {
    ...
    x <- read_csv(paste0("out/", 
      ...
      select(Foodgroup, outcome, outcome_unit, 
             ...
             `combined_consumed_upper_bound_median  (97.5th percentile)`,
             combined_unconsumed_impact_median,
             `combined_unconsumed_impact_lower_bound (2.5th percentile)`,
             `combined_unconsumed_upper_bound_median  (97.5th percentile)`,
...
my_costenv_dat1 <- temp %>% 
  ...
  rename(LB_day = `impact_lower_bound (2.5th percentile)`,
         ...
         LB_consumed_day = `combined_consumed_impact_lower_bound (2.5th percentile)`,
         UB_consumed_day = `combined_consumed_upper_bound_median  (97.5th percentile)`,
         LB_unconsumed_day = `combined_unconsumed_impact_lower_bound (2.5th percentile)`,
         UB_unconsumed_day = `combined_unconsumed_upper_bound_median  (97.5th percentile)`,
...
my_costenv_dat2 <- my_costenv_dat1 %>% 
  select(diet_pattern, food_group, outcome, impact_median, LB_day, UB_day, 
         combined_consumed_impact_median, LB_consumed_day, UB_consumed_day,
         combined_unconsumed_impact_median, LB_unconsumed_day, UB_unconsumed_day) %>%
  mutate(impact_median_year = impact_median * 365,
         LB_year = LB_day * 365,
         UB_year = UB_day * 365,
         combined_consumed_impact_median_year = combined_consumed_impact_median*365,
         LB_consumed_year = LB_consumed_day*365,
         UB_consumed_year = UB_consumed_day*365,
         combined_unconsumed_impact_median_year = combined_unconsumed_impact_median*365,
         LB_unconsumed_year = LB_unconsumed_day*365,
         UB_unconsumed_year = UB_unconsumed_day*365) %>% 
  rename(impact_median_day = impact_median, 
         combined_consumed_impact_median_day = combined_consumed_impact_median,
         combined_unconsumed_impact_median_day = combined_unconsumed_impact_median)
...
veg_temp <- full_comb2 %>% 
  ...
  summarise(impact_median_year = sum(impact_median_year),
            LB_year = sum(LB_year),
            UB_year = sum(UB_year),
            combined_consumed_impact_median_year = sum(combined_consumed_impact_median_year),
            LB_consumed_year = sum(LB_consumed_year),
            UB_consumed_year = sum(UB_consumed_year),
            combined_unconsumed_impact_median_year = sum(combined_unconsumed_impact_median_year),
            LB_unconsumed_year = sum(LB_unconsumed_year),
            UB_unconsumed_year = sum(UB_unconsumed_year)) %>% 
...
comb1 <- comb %>% 
  ...
  summarise(impact = sum(impact_median_year),
            ...
            consumed_impact = sum(combined_consumed_impact_median_year),
            consumed_impact_LB = sum(LB_consumed_year),
            consumed_impact_UB = sum(UB_consumed_year),
            unconsumed_impact = sum(combined_unconsumed_impact_median_year),
            unconsumed_impact_LB = sum(LB_unconsumed_year),
            unconsumed_impact_UB = sum(UB_unconsumed_year)) %>% 
  mutate(impact = ifelse(outcome == "Cancer (cases)" | outcome == "CVD (deaths)", impact * -1, impact),
         consumed_impact = ifelse(outcome == "Cancer (cases)" | outcome == "CVD (deaths)", consumed_impact * -1, consumed_impact),
         unconsumed_impact = ifelse(outcome == "Cancer (cases)" | outcome == "CVD (deaths)", unconsumed_impact * -1, unconsumed_impact))

comb2 <- comb1 %>% 
  pivot_wider(names_from = diet_pattern,
              values_from = c(impact, impact_LB, impact_UB, consumed_impact, consumed_impact_LB, consumed_impact_UB,
                              unconsumed_impact, unconsumed_impact_LB, unconsumed_impact_UB))
...
pc <- my_costenv_dat1 %>% 
  select(-c(impact_median, LB_day, UB_day, combined_consumed_impact_median, LB_consumed_day, UB_consumed_day, 
            combined_unconsumed_impact_median, LB_unconsumed_day, UB_unconsumed_day))
...
my_costenv_tot_dat2 <- my_costenv_tot_dat1 %>% 
  ...
  rename(impact_LB = `impact_lower_bound (2.5th percentile)`,
         ...
         impact_consumed_LB = `combined_consumed_impact_lower_bound (2.5th percentile)`,
         impact_consumed_UB = `combined_consumed_upper_bound_median  (97.5th percentile)`,
         impact_unconsumed_LB = `combined_unconsumed_impact_lower_bound (2.5th percentile)`,
         impact_unconsumed_UB = `combined_unconsumed_upper_bound_median  (97.5th percentile)`,
  ...
  mutate(impact_median_10perc = (impact_median / 10) * 365,
         ...
         impact_median_consumed_10perc = (combined_consumed_impact_median/10)*365,
         impact_consumed_LB_10perc = (impact_consumed_LB/10)*365,
         impact_consumed_UB_10perc = (impact_consumed_UB/10)*365,
         impact_median_unconsumed_10perc = (combined_unconsumed_impact_median/10)*365,
         impact_unconsumed_LB_10perc = (impact_unconsumed_LB/10)*365,
         impact_unconsumed_UB_10perc = (impact_unconsumed_UB/10)*365) 
...
my_health_tot_dat2 <- my_health_tot_dat1 %>% 
  ...
  mutate(impact_median_10perc = (impact_median / 10),
         ...
         combined_consumed_impact_median = NA,
         impact_consumed_LB = NA,
         impact_consumed_UB = NA,
         combined_unconsumed_impact_median = NA,
         impact_unconsumed_LB = NA,
         impact_unconsumed_UB = NA,
         impact_median_consumed_10perc = NA,
         impact_consumed_LB_10perc = NA,
         impact_consumed_UB_10perc = NA,
         impact_median_unconsumed_10perc = NA,
         impact_unconsumed_LB_10perc = NA,
         impact_unconsumed_UB_10perc = NA) 
...
a <- comb %>% 
  select(-c(impact_median_day, LB_day, UB_day, 
            combined_consumed_impact_median_day,LB_consumed_day, UB_consumed_day,
            combined_unconsumed_impact_median_day, LB_unconsumed_day, UB_unconsumed_day)) %>% 
  group_by(diet_pattern, outcome) %>% 
  mutate(sum = sum(impact_median_year),
         sum_LB = sum(LB_year),
         sum_UB = sum(UB_year),
         sum_consumed = sum(combined_consumed_impact_median_year),
         sum_consumed_LB = sum(LB_consumed_year),
         sum_consumed_UB = sum(UB_consumed_year),
         sum_unconsumed = sum(combined_unconsumed_impact_median_year),
         sum_unconsumed_LB = sum(LB_unconsumed_year),
         sum_unconsumed_UB = sum(UB_unconsumed_year)) %>% 
  arrange(diet_pattern, outcome)
...
a1 <- a %>% 
  mutate(prop = impact_median_year / sum * 100,
         ...
         prop_consumed = combined_consumed_impact_median_year / sum_consumed*100,
         prop_consumed_LB = LB_consumed_year / sum_consumed_LB*100,
         prop_consumed_UB = UB_consumed_year / sum_consumed_UB*100,
         prop_unconsumed = combined_unconsumed_impact_median_year / sum_unconsumed*100,
         prop_unconsumed_LB = LB_unconsumed_year / sum_unconsumed_LB*100,
         prop_unconsumed_UB = UB_unconsumed_year / sum_unconsumed_UB*100)
...
a1 %>% 
  ...
  summarise(sum(prop),
            ...
            sum(prop_UB),
            sum(prop_consumed),
            sum(prop_consumed_LB),
            sum(prop_consumed_UB),
            sum(prop_unconsumed),
            sum(prop_unconsumed_LB),
            sum(prop_unconsumed_UB))
