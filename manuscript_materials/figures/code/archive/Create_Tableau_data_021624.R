...
my_costenv_dat_ <- rbind(my_costenv_dat, my_fl_dat)

my_costenv_dat1 <- my_costenv_dat_ %>% 
...
# merge costenv with FL
my_costenv_tot_dat1 <- rbind(my_costenv_tot_dat, my_fl_tot_dat)
...
my_costenv_tot_dat3 <- my_costenv_tot_dat2 %>% 
  select(-c(diet_pattern, outcome, outcome_unit)) %>% 
...
# combine costenv and health
comb_new <- rbind(my_costenv_tot_dat3, my_health_tot_dat2) %>% 
...
results_10perc %>% filter(outcome == "FL (mrh-eq)") %>% mutate_at(vars(!outcome),
                                                                  .funs = funs((. / mil) %>% round(digits = 2))) %>% View()
