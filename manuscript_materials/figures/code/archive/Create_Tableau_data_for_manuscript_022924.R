...
# export
write_csv(comb, paste0("tables_figures/manuscript/data/abs_change_by_foodgroup_tableau_", export_date, ".csv"))
...
# export
write_csv(all, paste0("tables_figures/manuscript/data/perc_change_tableau_", export_date, ".csv"))
...
for (i in cra_folder_name) {
  ...
  x <- read_csv(paste0("out/", 
                       ...
                       "/CRA/", 
                       i, 
                       "/summarystats_attributable_USmortality_by_age_female_race_disease_type_2015_",
                       i,
                       ".csv")) %>% 
    select(age, female, race, disease_type, riskfactor, medians, LB, UB) %>% 
    mutate(diet_pattern = paste0(i)) %>% 
    relocate(diet_pattern)
...
# calculate percent change at the pattern-level
koo3 <- koo2 %>% 
  group_by(diet_pattern, outcome, age_gp, sex_gp, race_gp) %>% 
  summarise(total_impact_current = sum(current_intake_impact_median),
            total_LB_current = sum(current_intake_impact_LB),
            total_UB_current = sum(current_intake_impact_UB),
